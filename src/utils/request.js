 ...
 *  3xx:缓存
 *  4xx:客户端错误
 *      400:服务器收到客户端通过PUT或者POST请求提交的表示，表示的格式正确，但服务器不懂它什么意思(请求头错误、参数解析错误)
 *      401:客户端试图对一个受保护的资源进行操作，却又没有提供正确的认证证书(token验证失败)
 *      402:忽略
 *      403:客户端请求的结构正确，但是服务器不想处理它。这跟证书不正确的情况不同--若证书不正确，应该发送响应代码401
 *      404:资源被围定义(网络请求地址错误)
 *  5xx:服务器端错误
 *      500:执行请求处理代码时遇到了异常，它们就发送此响应代码(后台代码异常,有可能前端参数错误，后台处理不完善，导致异常)
 *      503:最可能的原因是资源不足：服务器突然收到太多请求，以至于无法全部处理(服务器崩溃)
 */
